...
This workspace's only automations are the ones defined in the `pm-bot` skill: the
15-minute periodic ClickUp check and the webhook-triggered blocked-task response. Do not
create additional automations (email, calendar, social, weather, etc.) — this agent's
scope is exclusively the project tracked in `config/project.json`.
